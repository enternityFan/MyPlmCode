...
class ELMoLstmEncoder(nn.Module):
    ...
    def forward(self,inputs,lengths):
        ...
        for layer_index in range(self.num_layers):
            ...
            forward = pack_padded_sequence(packed_forward,batch_first=True)[0]
            forward = self.forward_projections[layer_index](forward)
            stacked_backward_states.append(forward)
...
class BiLM(nn.Module):
    ...
    def __init__(self,configs,vocab_w,vocab_c):
        ...
        self.classifier = nn.Linear(configs['projection_dim'],self.num_class)
